#!/usr/bin/env python3
"""Generate the survey definition for the im_model CSEM inversion benchmark.
...
Outputs (../survey/):
    ...
    sigmas_true.txt    conductivities for im_true.msh (AIR, BG, INVERT, ANOMALY)
    sigmas_im.txt     starting model for im_model.msh (AIR, BG, INVERT), with
...
SRC = (0.0, -4000.0, 0.0)                             # x, y, z  (z positive down)
...
def main():
    ...
    with open(_p("receivers.txt"), "w") as f:
        f.write("# 441 (21x21) receivers, x,y in [-200,200] m, 20 m spacing, "
                "z = 0.\n# z is positive down: z=0 is the air/earth interface.\n"
                "# x y z\n")
        for x, y, z in pts:
    ...
    # True model, on im_true.msh -> tags AIR(1) BG(2) INVERT(3) ANOMALY(4).
    with open(_p("sigmas_true.txt"), "w") as f:
        f.write("# TRUE model conductivities for im_true.msh --> fm.csem.\n"
    ...
    # Inversion starting model, on im_model.msh -> tags AIR(1) BG(2) INVERT(3).
    # Uniform 100 ohm-m halfspace; only the INVERT region is free.
    with open(_p("sigmas_im.txt"), "w") as f:
        f.write("# INVERSION starting model for im_model.msh --> im.csem.\n"
