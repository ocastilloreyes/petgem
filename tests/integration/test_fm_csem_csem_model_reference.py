...
def test_fm_csem_csem_model_reference(repo_root, fm_csem_binary, reference_workspace):
    ...
    responses_path, _ = run_pipeline_for_nord(
        repo_root, fm_csem_binary, reference_workspace, NORD
    )
    ...
    assert Ex_sim.shape == Ex_ref.shape, (
        f"PETGEM Ex shape {Ex_sim.shape} vs reference {Ex_ref.shape}"
    )
    ...
    assert nrmsd_val < NRMSD_TOLERANCE, (
        f"csem_model NRMSD {nrmsd_val:.6e} exceeded tolerance {NRMSD_TOLERANCE}"
    )
